...
--[[
    pages.lua: calculates page numbers for printing books. Written because I got sick of doing it by hand. Ver. 1.2.
...
-- Extracting number of pages and signature size from input (arg[1] and arg[2] respectively)
local n_pages, sig_size = math.tointeger(arg[1]), math.tointeger(arg[2])

-- Computing the number of signatures
local n_sig = n_pages / sig_size

--[[
    Error handling: it isn't optimal to make books with signatures of less than 12 or greater than 20 pages,
    so here we consider any number, other than 12, 16 or 20 to be invalid input. Furthermore, he number of
    pages should be divisible by signature size. Otherwise, there will be signatures of different sizes
--]]
if sig_size ~= 12 and sig_size ~= 16 and sig_size ~= 20 then
    error "Incorrect input data: signature size must be either 12, 16 or 20"
elseif n_sig % 1 ~= 0 then
    error "Incorrect input data: number of pages isn't divisible by signature size"
end

--[[ **Building the array of numbers** ]]--

-- Defining an auxiliary variable (to simplify formulas) and the array with signatures
local s, signatures = sig_size // 2, {}

-- Building the array. For information about the algorithm, see below
for i = 1, n_sig do
    signatures[i] = {{}, {}}
    for j = 1, s do
        signatures[i][1][j] = math.tointeger((sig_size * (i - 1) + s + (-1)^j * (j - s - 1) - 2) % n_pages)
        signatures[i][2][j] = math.tointeger((sig_size * (i - 1) + s + (-1)^j * (j - 1) - 3) % n_pages + 1)
    end
end
...
for i = 1, n_sig do
    ...
    io.write("Side 1: ", tostring(table.concat(signatures[i][1], ", ")), "\n")
    io.write("Side 2: ", tostring(table.concat(signatures[i][2], ", ")), "\n\n")
end
